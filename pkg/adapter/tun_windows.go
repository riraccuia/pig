 ...
 *
 * Portions of this file are derived from wintun-go
 * (https://git.zx2c4.com/wintun-go/tree/).
 */

// Modifications Copyright 2026 Riccardo Raccuia
//
...
func (tun *NativeTun) IPNet() *net.IPNet {
	return tun.ip
}

func (tun *NativeTun) IPNet6() *net.IPNet {
	return tun.ipv6
}
...
func (tun *NativeTun) Write(buf []byte) (int, error) {
	...
	tun.rate.update(uint64(len(buf)))

	packet, err := tun.session.AllocateSendPacket(len(buf))
	switch err {
	case nil:
	...
	case windows.ERROR_BUFFER_OVERFLOW:
		return 0, nil // Dropped when ring is full.
	default:
